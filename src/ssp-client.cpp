...
void SSPClient::doStart()
{
	...
	this->threadLoop = new imf::ThreadLoop(
		std::bind(SSPClient::PreStart, this, _1), create_loop_class);
...
void SSPClient::PreStart(SSPClient *my, imf::Loop *loop)
{
	...
	my->implLock.lock();
	my->impl = create_ssp_class(my->ip, loop, my->bufferSize, 9999,
				    imf::STREAM_DEFAULT);
	my->impl->init();
	...
	if (my->connectedCallback) {
		my->impl->setOnConnectionConnectedCallback(
...
void SSPClient::setOnRecvBufferFullCallback(
...
void SSPClient::setOnConnectionConnectedCallback(
